...
A token is used to recognize a returning user. The token is “given” to the user in exchange for the correct login credentials.

- [x] What steps can you take in your web apps to keep your data secure?

Steps that can be taken to keep data secure are using private routes for sensitive user data and requiring an authentication for users to view their data.

- [x] Describe how web servers work.

Web servers store code for different websites that can be retrieved to display a website by a user’s request. 

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

Put, Get and Delete can be mapped to the CRUD acronym.
	Create = PUT/POST
	Read = GET
	Update = PUT
	Delete = DELETE
